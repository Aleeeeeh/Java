...
public class program {
	public static void main(String args[]) throws ParseException {
		...
		System.out.println("Quantos contratos são desse trabalhador: ");
		int n = sc.nextInt();
		//Esse for irá repetir todos dados do trabalhador e contratos
		for (int i=1; i<n; i++) {
		...
		System.out.println("---------------------------");
		System.out.print("Entre com Mês e Ano para calculo da renda (MM/yyyy: ");
		String mes_ano = sc.next();
		int mes = Integer.parseInt(mes_ano.substring(0,2)); //Recorta do 0 a 2
		int ano = Integer.parseInt(mes_ano.substring(3)); // Recorta da 3 posição em diante
		System.out.println("Nome: " + trabalhador.getNome()); //Pegar nome do objeto trabalhador
		System.out.println("Departamento: " + trabalhador.getDepartamento().getNome());//Composição de objeto
		System.out.println("Renda do Mês" + mes_ano + ": " + String.format("%.2f", trabalhador.renda(mes, ano)));
